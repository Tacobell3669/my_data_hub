...
over_time_counts = cur.fetchall()
over_time_counts_df = pd.DataFrame(over_time_counts, columns=['state', 'species_listing_count', 'listing_year'])
...
over_time_counts_df['state_code'] = over_time_counts_df['state']

# Transform the count data into a format that can be used by Chloropleth
over_time_counts_df['species_listing_count'] = over_time_counts_df['species_listing_count'].astype(float)
over_time_counts_df['listing_year'] = over_time_counts_df['listing_year'].astype(str)
over_time_counts_df.drop('state', axis=1, inplace=True)

# Create full data set of all states and all years
years = pd.date_range(start='1967', end='2023', freq='Y')
years = years.strftime('%Y')
state_codes = over_time_counts_df['state_code'].unique()
...
df_new = pd.merge(df_new, over_time_counts_df, how='outer', left_on=['state_code', 'listing_year'],
...
# Create an over time Chloropleth map using Plotly and the dataframe we made
